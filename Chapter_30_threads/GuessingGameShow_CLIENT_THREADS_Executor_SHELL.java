...
public class GuessingGameShow_CLIENT_THREADS_Executor_SHELL
{
    ...
    {    
     ...
     // Declare the ExecutorService naming it 'executor' and define it to be a newFixedThreadPool with size 'numberOfPlayers'
             // >>>>> YOUR CODE HERE <<<<<
     // Declare GuessingGameShow_PlayerThread named 'threadArray' and define it to be of size 'numberOfPlayers'
             // >>>>> YOUR CODE HERE <<<<<

        for (int i=1; i<=numberOfPlayers; i++)
        {
            n = (int) (Math.random()*max+1);
            p = new GuessingGameShow_PlayerThread("P"+i,n,max);   
